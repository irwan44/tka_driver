...
class API {
  ...
  static String get _PutKonfirmasiPlanning => '$_url/driver/service/konfirmasi/';

  static Future<void> _handleTokenExpiration(int statusCode, String responseBody) async {
    if (statusCode == 401) {
  ...
  static Future<Profile2> getProfile() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _handleTokenExpiration(response.statusCode, response.body);
        throw Exception('Gagal mengambil profile (code: ${response.statusCode}) -> ${response.body}');
      }
    } on SocketException catch (e) {
      print("No internet connection: $e");
      ...
      if (cachedProfile.isNotEmpty) {
        ...
        throw Exception("Tidak ada koneksi internet dan data cache tidak tersedia");
      }
    }
  }
  ...
  static Future<List<RequestService>> fetchListRequestService() async {
    try {
      ...
      if (r.statusCode == 200) {
        ...
        return (data as List)
            .map((e) => RequestService.fromJson(e))
            .toList();
      } else {
    ...
    try {
      ...
      if (streamedResponse.statusCode == 200 ||
        ...
        await _handleTokenExpiration(
            streamedResponse.statusCode, responseBody.body);
        throw Exception(
  ...
  static Future<List<ListService>> fetchListService() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _handleTokenExpiration(response.statusCode, response.body);
        throw Exception('Failed to fetch list service (code: ${response.statusCode}) -> ${response.body}');
      }
    } on SocketException catch (e) {
  ...
  static Future<DetailService> fetchDetailService(String kodeSvc) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _handleTokenExpiration(response.statusCode, response.body);
        throw Exception('Gagal mengambil detail service (code: ${response.statusCode}) -> ${response.body}');
      }
    } on SocketException catch (e) {
      print("No internet connection: $e");
      throw SilentException("Tidak ada koneksi internet");
  ...
  static Future<MekanikPosisi> fetchMekanikPosisi(String kode) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _handleTokenExpiration(response.statusCode, response.body);
        throw Exception("Gagal mengambil posisi mekanik (code: ${response.statusCode})");
      }
    } on SocketException catch (e) {
      print("No internet connection: $e");
      throw SilentException("Tidak ada koneksi internet");
