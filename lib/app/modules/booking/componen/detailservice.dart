...
class _DetailServiceViewState extends State<DetailServiceView> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: isPlanning
          ? Obx(() {
        final confirmed = controller.isPlanningConfirmed(widget.kodeSvc);
      ...
      body: RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          child: Obx(() {
            ...
            final kodeValue =
            isEstimasi ? _safe(d.dataSvc?.kodeEstimasi) : _safe(d.dataSvc?.kodePkb);
              ...
              children: [
                TicketCard(
                  ...
                  child: Column(children: [
                    _twoCol("Kode Svc", _safe(d.dataSvc?.kodeSvc),
                        kodeLabel, kodeValue),
                    _twoCol("Tipe Svc", _safe(d.dataSvc?.tipeSvc),
                        "Keluhan", _safe(d.dataSvc?.keluhan)),
                    _twoCol("Tgl Keluar", _safe(d.dataSvc?.tglKeluar),
                        "Tgl Kembali", _safe(d.dataSvc?.tglKembali)),
                  ]),
                ),
                ...
                TicketCard(
                  ...
                  child: Column(children: [
                    _twoCol("No Polisi", _safe(d.dataSvc?.noPolisi),
                        "Merk", _safe(d.dataSvc?.namaMerk)),
                    _twoCol("Tipe", _safe(d.dataSvc?.namaTipe),
                        "Tahun", _safe(d.dataSvc?.tahun)),
                    _twoCol("Warna", _safe(d.dataSvc?.warna),
                        "Transmisi", _safe(d.dataSvc?.transmisi)),
                    _twoCol("Odometer", _safe(d.dataSvc?.odometer),
                        "Kategori", _safe(d.dataSvc?.kategoriKendaraan)),
                    _twoCol("No Rangka", _safe(d.dataSvc?.noRangka),
                        "No Mesin", _safe(d.dataSvc?.noMesin)),
                  ]),
                ),
                const SizedBox(height: 12),
                TicketCard(
                  ...
                  child: _twoCol("PIC", _safe(d.dataSvc?.pic),
                      "HP PIC", _safe(d.dataSvc?.hpPic)),
                ),
                const SizedBox(height: 20),
                TicketCard(
                  title: "Paket Service",
                  iconData: Icons.build_circle_outlined,
                  child: (d.dataSvcPaket?.isNotEmpty ?? false)
                      ? Column(
                    children: d.dataSvcPaket!.map((p) => Padding(
                      padding:
                      const EdgeInsets.symmetric(vertical: 6),
                      child: TicketCardItem(children: [
                        _detailRow(Icons.code, "Kode", _safe(p.kode)),
                        _detailRow(Icons.label, "Nama", _safe(p.nama)),
                        _detailRow(Icons.format_list_numbered, "Qty",
                            _safe(p.qty)),
                        _detailRow(Icons.attach_money, "Harga",
                ...
                TicketCard(
                  ...
                  child: (d.dataSvcDtlPart?.isNotEmpty ?? false)
                      ? Column(
                    children: d.dataSvcDtlPart!.map((part) => Padding(
                      padding:
                      const EdgeInsets.symmetric(vertical: 6),
                      child: TicketCardItem(children: [
                        _detailRow(Icons.extension, "Nama Sparepart",
                ...
                TicketCard(
                  ...
                  child: (d.dataSvcDtlJasa?.isNotEmpty ?? false)
                      ? Column(
                    children: d.dataSvcDtlJasa!.map((j) => Padding(
                      padding:
                      const EdgeInsets.symmetric(vertical: 6),
                      child: TicketCardItem(children: [
                        _detailRow(Icons.handyman, "Nama Jasa",
                            _safe(j.namaJasa)),
                        _detailRow(Icons.format_list_numbered, "Qty",
                            _safe(j.qtyJasa)),
                        _detailRow(Icons.attach_money, "Harga",
  ...
  Widget _twoCol(String l1, String v1, String l2, String v2) {
    ...
    TextStyle val =
    GoogleFonts.nunito(fontSize: 15, color: dark ? Colors.white : Colors.black87);
    ...
    return Padding(
      ...
      child: Row(children: [
        Expanded(
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [Text(l1, style: label), const SizedBox(height: 4), Text(v1, style: val)]),
        ),
        const SizedBox(width: 16),
        Expanded(
          child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [Text(l2, style: label), const SizedBox(height: 4), Text(v2, style: val)]),
        ),
      ]),
    );
  }
    ...
    return Padding(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(
            child: Text(value,
                style: GoogleFonts.nunito(
                    fontSize: 15, color: dark ? Colors.white : Colors.black87))),
      ]),
    );
  }
  ...
  Widget _buildShimmer(bool dark) {
    ...
    return Shimmer.fromColors(
      ...
      highlightColor: hi,
      child: Column(children: [
        TicketCard(title: "Informasi Utama", iconData: Icons.info_outline, child: bar(6)),
        const SizedBox(height: 12),
        TicketCard(title: "Detail Kendaraan", iconData: Icons.directions_car, child: bar(10)),
        const SizedBox(height: 12),
        TicketCard(title: "Detail PIC", iconData: Icons.person_outline, child: bar(2)),
      ]),
    );
  }
}
...
class TicketCard extends StatelessWidget {
  ...
  const TicketCard({Key? key, required this.title, required this.child, this.iconData = Icons.receipt})
      : super(key: key);
      ...
      child: ClipPath(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Container(
              decoration: BoxDecoration(
                color: dark ? Colors.blueGrey.withOpacity(0.3) : Colors.blue.withOpacity(0.1),
                borderRadius: BorderRadius.circular(8),
              ),
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(iconData, size: 20, color: dark ? Colors.white70 : Colors.blueGrey[700]),
                const SizedBox(width: 6),
                Text(title,
                    style: GoogleFonts.nunito(
                        fontSize: 16, fontWeight: FontWeight.bold, color: dark ? Colors.white : Colors.blueGrey[800])),
              ]),
            ),
